...
class Canvas:
    def __init__(self, image, drawing_mode, update_streamlit=True):
        ...
        self.json_data = None
        self.key = None
        self.scaled_coordinates = pd.DataFrame(columns=["Left", "Top", "Width", "Height", "scaleX", "scaleY"])
    ...
    def process_drawing_coordinates(self):
    ...
    def get_scaled_coordinates(self, table_objects, page, table_titles):
        if len(table_objects) > 0:
            ...
            for _, row in table_objects.iterrows():
                ...
                try:
                    ...
                    title = f'Extra data {table_count}'  # Si no hay suficientes títulos, establece un título genérico
