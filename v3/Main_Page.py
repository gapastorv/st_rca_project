...
def signup_action(mail, password, name):
    ...
    if record is not None:
        ...
        if cond == 1:
            st.error("Email already exists in website")
        if cond == 2:
            st.error("Username already exists")
        if cond == 3:
            st.error("Something went wrong")
        if cond == -1:
            st.error("Email domain not allowed, just gmail and outlook allowed")
...
def main_page():
    st.title("Main Page")
    st.write("Welcome to the data extractor web page")
